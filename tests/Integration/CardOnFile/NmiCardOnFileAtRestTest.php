...
/**
 * What a card on file looks like on disk.
 *
 * The same promise the stored cards make, kept by a second encrypter: the vault reference, the
 * billing identifier and the verifying transaction are credentials and must be unreadable in the
 * row. Read straight from the row, because the ORM decrypts on load and reading the entity back
 * would prove nothing.
 */
final class NmiCardOnFileAtRestTest extends KernelTestCase
{
    use BuildsAnNmiPaymentRequest;

    private const TOKENIZATION_KEY = 'tok-public-at-rest';

    private const SECURITY_KEY = 'sec-private-at-rest';
    ...
    {
        $card = $this->aCardOnFile();
        $this->manager->flush();
        $this->manager->clear();

        /** @var NmiCardOnFileInterface $reloaded */
        $reloaded = self::getContainer()->get('jpm_martin_sylius_nmi.repository.nmi_card_on_file')->find($card->getId());
    ...
    private function storedRow(): array
    {
        ...
        $row = $this->manager->getConnection()->fetchAssociative(
            'SELECT vault_id, billing_id, initial_transaction_id, brand, last_four, expiry_month, expiry_year
             FROM jpm_martin_sylius_nmi_card_on_file WHERE id = :id',
            ['id' => $card->getId()],
        );
    ...
    private function aCardOnFile(): NmiCardOnFileInterface
    {
        ...
        $card->setInitialTransactionId(self::INITIAL_TRANSACTION_ID);
        $card->setBrand('Visa');
