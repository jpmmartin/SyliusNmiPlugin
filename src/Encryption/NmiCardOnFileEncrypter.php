...
/**
 * Encrypts the three gateway identifiers on a card on file and nothing else — the same fields, for
 * the same reason, as the stored-card encrypter does. The platform's encrypter does the work.
 *
 * @implements EntityEncrypterInterface<NmiCardOnFileInterface>
 *
 * @internal
 */
final readonly class NmiCardOnFileEncrypter implements EntityEncrypterInterface
{
    ...
    {
        if (!$resource instanceof NmiCardOnFileInterface) {
            return;
        }
